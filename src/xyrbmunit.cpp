...
std::vector<double> xyRBMUNIT::Get_v_bias()
{
    std::vector<double> P;
    P.clear();
    ...
    for(unsigned int k=0;k<vsize;k++){
      P.at(k)=xyvneurons.at(k).Get_bias();
    }
...
std::vector<double> xyRBMUNIT::Get_h_bias()
{
    std::vector<double> P;
    P.clear();
    ...
    for(unsigned int j=0;j<hsize;j++){
      P.at(j)=xyhneurons.at(j).Get_bias();
    }
...
int xyRBMUNIT::rbm_train_one_batch(const std::vector<std::vector<double>> &data)
{
    ...
    for(unsigned int i=0;i<numcases;i++){
      tempv.clear();
      for(unsigned int j=0;j<hsize;j++){
        b=xyhneurons.at(j).Get_p_prob(i);
        tempv.push_back(b);
      }
      poshidprobs.push_back(tempv);
    }
    ...
    std::cout << "poshidprobs stored" << std::endl;
    for(unsigned int i=0;i<numcases;i++){
      for(unsigned int j=0;j<hsize;j++){
        std::cout << poshidprobs.at(i).at(j) << " ";
      }
      std::cout << std::endl;
    }
    */    
