...
void xy_Autoencoder::Set_net_strucuture(const std::vector<unsigned int> &st)
{
    ...
    std::cout << "Weight " << sizep << " initialized randomly." << std::endl;
    ...
    */
    ...
    for(unsigned int ne=0;ne<nep;ne++){  //epoch loop
      ...
      for(unsigned int nb=0;nb<sizenb;nb++){  //mini batch loop
        ...
        std::cout << "eval probs and ds" << std::endl;
...
void xy_Autoencoder::read_W(std::string &pref)
{
    Weights.clear();
    ...
    std::cout << "File pref: " << pref << std::endl;
    unsigned int sizew;
    std::ifstream infile(pref);
    if(!infile.is_open()){
      std::cout << "File sets pref not exist." << std::endl;
      return;
    }
    infile >> sizew;
    infile.close();
    std::cout << "# of files= " << sizew << std::endl;

    Weights.resize(sizew);
    ...
    for(unsigned int i=1;i<sizew;i++){
      ...
      infile >> nx >> ny;
      std::cout << "  W size: " << nx << " X " << ny << std::endl;
      ...
      Weights.at(i).W_in(tempm);

      infile >> nx >> ny;
      std::cout << "  Bias size: " << nx << " X " << ny << std::endl;
